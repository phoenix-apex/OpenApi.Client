...
namespace DA.Systems.Cube.Norsk.Client
{
    ...
    public partial class ApiClient : IDisposable, ISynchronousClient, IAsynchronousClient
    {
        ...
        /// </summary>
        public ApiClient() : this(DA.Systems.Cube.Norsk.Client.GlobalConfiguration.Instance.BasePath)
        {
        }
        ...
        /// </remarks>
        public ApiClient(HttpClient client, HttpClientHandler handler = null) :this(client, DA.Systems.Cube.Norsk.Client.GlobalConfiguration.Instance.BasePath, handler)
        {
        }
            ...
            System.Threading.CancellationToken cancellationToken = default(System.Threading.CancellationToken))
        {
            ...
            try
            {
                ...
                if (typeof(DA.Systems.Cube.Norsk.Model.AbstractOpenAPISchema).IsAssignableFrom(typeof(T)))
                {
        ...
        private string ComputeAuthHash(HttpMethod httpVerb, string rawJson, string contentType, string endpoint, DateTimeOffset dateTime, string secretAccessKey = null) {
            if(string.IsNullOrWhiteSpace(secretAccessKey)) secretAccessKey = GlobalConfiguration.Instance.SecretAccessKey;
            using var md5 = MD5.Create();
            var authBuilder = new StringBuilder();
            authBuilder.Append(httpVerb).Append("\n");
            if (string.IsNullOrWhiteSpace(rawJson) || httpVerb == HttpMethod.Get) authBuilder.Append("\n");
            else
                authBuilder.Append(BitConverter.ToString(md5.ComputeHash(Encoding.UTF8.GetBytes(rawJson))).ToLowerInvariant()
                    .Replace("-", "")).Append("\n");
            ...
            var authBody = authBuilder.ToString();
            using var hmacsha1 = new HMACSHA1();
            hmacsha1.Key = Encoding.UTF8.GetBytes(secretAccessKey);
            var signBytes = Encoding.UTF8.GetBytes(authBody);
